...
date = 2022-07-02
aliases = ["/lattices-for-programmers-pt1/"]
[taxonomies]
series = ["lattices-for-programmers"]
tags = ["lattices", "cryptography"]
[extra]
series_order = 1
+++
...
Anyway, {% mi() %}\mathsf{SVP}_1{% end %} is (under some reasonable technical assumptions) NP-hard.
...
This is wrong though --- formally, {% mi() %}\mathsf{SVP}_{\Omega(\sqrt{n})}{% end %} is known to be in NP and coNP, so (under even more technical assumptions [[^PHcollapse]]) *can’t* be NP-hard.

[^PHcollapse]: Formally, it is not NP-hard unless the polynomial hierarchy collapses all the way.

There are other caveats about concretely applying the aforementioned connection to {% mi() %}\mathsf{SVP}_\gamma{% end %} (that have to do with “tightness” of the reduction).
