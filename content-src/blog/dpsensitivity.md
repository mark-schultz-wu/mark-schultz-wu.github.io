...
{% md() %}\mathsf{mean}(\vec{x}) = \frac{\sum_{i = 0}^{122} i \vec{x}_i}{\sum_{i = 0}^{122} \vec{x}_i} \sim \frac{123\mathbb{E}[\vec x]}{\lVert \vec x\rVert_1}{% end %}
...
I’ve written up some details [here](@/blog/sensitivity-calculus.md), but “short story” is that the chain rule one can derive involves terms of the form:
