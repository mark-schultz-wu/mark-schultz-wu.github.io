...
date = 2022-07-02
aliases = ["/lattices-for-programmers-pt2/"]
[taxonomies]
...
First, one might be tempted define {% mi() %}\mathsf{Enc}_{\vec s}(\vec m) = (A, A\vec s \oplus \vec m){% end %}, where $\oplus$ is an operation such as XOR.
...
Namely, under the (false) Noiseless LWE assumption, one can show that {% mi() %}\mathsf{Enc}_{\vec s}(\vec m) := (A, A\vec s+\vec m){% end %} defines a secure encryption function.
...
{% md() %}\mathsf{Dec}_{\vec s}(\mathsf{Enc}_{\vec s}(\vec m)) = \mathsf{Dec}_{\vec s}(A, A\vec s + \vec e +\vec m) = \vec m + \vec e\neq \vec m.{% end %}
